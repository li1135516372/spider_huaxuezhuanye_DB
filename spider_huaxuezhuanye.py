...
import re
import os
#"C:/Users/Administrator/Desktop/数据样本/中药材/data_2.txt"
def get_medicine_name(path):  #获取中药材列表
    file = open(path, 'r')
    line = file.readlines()
    str_medicine = line[0]
...
def get_load():
    ...
    if (medicine_name.index(i) % 5 == 0):
        username.send_keys('3li1135516372')
    if (medicine_name.index(i) % 5 == 1):
        username.send_keys('li11355163724')
    if (medicine_name.index(i) % 5 == 2):
        username.send_keys('li11355163725')
    if (medicine_name.index(i) % 5 == 3):
        username.send_keys('li11355163726')
    if (medicine_name.index(i) % 5 == 4):
        username.send_keys('li11355163727')
    time.sleep(random.uniform(5, 6))
    try:
        password = browser.find_element_by_name('Password')
    except:
        fail_medicine.append(i)
    password.send_keys('li123456789')
    time.sleep(random.uniform(5, 6))
    try:
...
if __name__ == '__main__':
    ...
    url1 = 'http://www.organchem.csdb.cn/scdb/Tcm_Multi/q_tcd.asp'
